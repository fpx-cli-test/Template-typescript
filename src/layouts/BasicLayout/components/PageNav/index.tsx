  ...
  return menusData
    .filter((item) => {
      ...
      if (auth && item.auth && item.auth instanceof Array) {
        if (item.auth.length) {
          roleAuth = item.auth.some((key: string | number) => auth[key]);
        }
      }
...
const Navigation = (props: { location: any; }, context: { isCollapse: any; }) => {
  ...
  useEffect(() => {
    const curSubNav = asideMenuConfig.find((menuConfig: { children: any; }) => {
      return menuConfig.children && checkChildPathExists(menuConfig);
    });

    function checkChildPathExists(menuConfig: { children: any[]; }) {
      return menuConfig.children.some((child: { children: any; path: any; }) => {
